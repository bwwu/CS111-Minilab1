...
do_fork(process_t *parent)
{
	...
	// Find a entry in process table to place new process
	do {
		++i;
		if (i >= NPROCS)
			return -1;
		child = proc_array+i;
	} while(child->p_state != P_EMPTY);
	
	// Copy registers from parent to child, set return value (eax) to 0
	memcpy(&(child->p_registers), &(parent->p_registers), sizeof(registers_t));
	child->p_registers.reg_eax = 0;
	// Copy parent stack to child and mark new process as runnable
	child->p_state = P_RUNNABLE; 
	copy_stack(child,parent);
	return i;		//Return pid of child to its parent
}
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// YOUR CODE HERE!
	int32_t src_pid = src->p_pid;
	int32_t dest_pid = dest->p_pid;
	int32_t stack_size;

	src_stack_top = PROC1_STACK_ADDR + src_pid*PROC_STACK_SIZE;
	src_stack_bottom = src->p_registers.reg_esp;

	// Determine current size of src stack	
	stack_size = src_stack_top - src_stack_bottom;
	
	// Determine stack bounds for dest proc
	dest_stack_top = PROC1_STACK_ADDR + dest_pid*PROC_STACK_SIZE;
	dest_stack_bottom = dest_stack_top - stack_size; 
	
	// Set dest esp to bottom of its stack
